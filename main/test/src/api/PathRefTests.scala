    ...
    test("json") {
      def check(quick: Boolean) = withTmpDir { tmpDir =>
        ...
        if (quick) {
          ...
          assert(json.endsWith(s""":root:${file}""""))
        } else {
          val expected = s""""ref:4c7ef487:root:${file}""""
      ...
      test("ctx") {
        def check(quick: Boolean) = withTmpDir { tmpDir =>
          ...
          val json = PathRef.pathRefContext.withValue(Seq(("workspace" -> tmpDir))) {
            val json = upickle.default.write(pr)
            if (quick) {
              assert(json.startsWith(""""qref:"""))
              assert(json.endsWith(s""":workspace:foo.txt""""))
            } else {
              val expected = s""""ref:4c7ef487:workspace:foo.txt""""
              assert(json == expected)
            }
            val pr1 = upickle.default.read[PathRef](json)
            assert(pr == pr1)
          ...
          // outside of defined context
          val ex = intercept[RuntimeException] {
            upickle.default.read[PathRef](json)
          }
          assert(ex.getMessage == "Unsupported PathRef context 'workspace' found")
        }
