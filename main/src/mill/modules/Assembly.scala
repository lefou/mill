...
object Assembly {
  ...
  object Rule {
    case class Append private (path: String, separator: String) extends Rule {
      private[modules] def copy(path: String = path, separator: String = separator): Append =
    ...
    class AppendPattern private (val pattern: Pattern, val separator: String) extends Rule {
      ...
      private[modules] def copy(pattern: Pattern = pattern): AppendPattern =
    ...
    case class Exclude private (path: String) extends Rule {
      private[modules] def copy(path: String = path): Exclude = new Exclude(path)
    }
    ...
    case class Relocate private (from: String, to: String) extends Rule {
      private[modules] def copy(from: String = from, to: String = to): Relocate = new Relocate(from, to)
    }
    ...
    case class ExcludePattern private (pattern: Pattern) extends Rule {
      private[modules] def copy(pattern: Pattern = pattern): ExcludePattern = new ExcludePattern(pattern)
    }
