...
object Deps {
  ...
  val cmdoption = ivy"de.tototec:de.tototec.cmdoption:0.7.0"
...
object main extends MillModule {
  ...
  object client extends MillPublishModule{
    ...
    object universal extends MillApiModule {
      override def ivyDeps = Agg(Deps.cmdoption)
      override def moduleDeps: Seq[PublishModule] = Seq(main, main.client)
      override def generatedSources: Target[Seq[PathRef]] = T{
        val dest = T.ctx().dest
        val body =
          s"""package mill.main.client.universal;
             |/** Build-time generated by mill. */
             |public class Versions {
             |  /** Build-time mill version */
             |  public static String millVersion() { return "${publishVersion()}"; }
             |}
             |""".stripMargin
        os.write(dest / "mill" / "main" / "client" / "universal" / "Versions.java", body, createFolders = true)
        Seq(PathRef(dest))
      }
      object test extends Tests {
  ...
  mill.modules.Jvm.universalScript(
    shellCommands = {
      ...
      s"""if [ -z "$$JAVA_HOME" ] ; then
         ...
         |fi
         |${java("mill.main.client.universal.MillUniversalClient")}
         |""".stripMargin
    },
    cmdCommands = {
      ...
      s"""set "JAVACMD=java.exe"
         ...
         |if "%1" == "-i" set _I_=true
         |${java("mill.main.client.universal.MillUniversalMain")}
         |""".stripMargin
    }
  )
}
...
object dev extends MillModule{
  def moduleDeps = Seq(scalalib, scalajslib, scalanativelib, main.client.universal)
